/**
 * Type T 导出渲染模块（满幅无边框 / 参考图）
 * 布局：照片铺满整张画布（无白边）+ 左上两行文字 + 右侧竖排日期与针状标记 + 右下参数行
 * - 照片比例可切换（1:1 / 1:1.35 纵向），画布 = 照片比例
 * - 照片以 cover 铺满画布并按归一化偏移裁剪（与预览一致）
 * - 文字带淡阴影（canvas shadow），保证压在照片上可读
 * - 布局常量与预览侧 type-T-preview.js 共用
 */
...
function drawTexts(ctx, W, H, settings, color) {
  ...
  ctx.shadowBlur = Math.max(1, L.shadowBlur * W);
  ctx.shadowOffsetY = Math.max(0.5, L.shadowOffsetY * W);
