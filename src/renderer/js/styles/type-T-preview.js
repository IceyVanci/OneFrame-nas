/**
 * Type T 预览样式模块（满幅无边框 / 参考图）
 * 布局：照片铺满整张画布（无白边）+ 左上两行文字 + 右侧竖排日期与针状标记 + 右下参数行
 * - 照片比例可切换（1:1 / 1:1.35 纵向），画布 = 照片比例
 * - 照片以 cover 铺满画布，并按归一化偏移拖动
 * - 左上：第一行（默认空）+ 第二行「PHOTO BY {handle}」
 * - 右侧：竖排日期（YYYYMMDD，自上而下）+ 针状标记（宽 1em、长 5em、尖端渐隐），垂直居中
 * - 右下：ISO / 焦距 / 光圈 / 快门 一行（EXIF 预填，可手改）
 * - 文字颜色：白色（默认）/ 图片主色 / 自定义（点选图片或原生选色器）
 * - 文字带淡阴影，保证压在照片上可读
 * - 忽略：右上装饰标题、左下 Logo、底部圆点
 */

import { getDominantColor } from '../utils/dominant-color.js';

export const TYPE_T_LAYOUT = {
  ...
  shadowBlur: 0.003,       // 文字阴影模糊（× 画布宽，导出用）
  shadowOffsetY: 0.0012,   // 文字阴影 Y 偏移（× 画布宽，导出用）
  shadowColor: 'rgba(0,0,0,0.45)' // 阴影颜色
};
...
/**
 * 比例字符串 → 高/宽
 * @param {string} ratio - '1:1' | '1:1.35'
 * @returns {number}
 */
function ratioHOverW(ratio) {
  return ratio === '1:1.35' ? 1.35 : 1;
}

/**
 * 计算画布尺寸（满幅且不放大：高 = min(原图高, 原图宽 × 高宽比)）
 * @param {Object} settings - { naturalWidth, naturalHeight, ratioT }
 * @returns {Object} { squareSize, canvasHeight }
 */
export function calcSize(settings = {}) {
  const { naturalWidth = 0, naturalHeight = 0 } = settings;
  const ratio = settings.ratioT === '1:1.35' ? '1:1.35' : '1:1';
  state.ratio = ratio;

  const aHW = ratioHOverW(ratio);            // 高/宽
  const canvasHeight = Math.min(naturalHeight, naturalWidth * aHW);
  const canvasWidth = canvasHeight / aHW;
...
export function updateFrameWrapper(squareSize, canvasHeight) {
  ...
  state.frameWrapper.classList.add('type-t');
...
export function resolveTextColor(settings = {}, img = state.img) {
  const choice = settings.textColorT || '#ffffff';
...
export function buildParamsLine(settings = {}) {
  const segs = [];
  const iso = String(settings.isoT || '').trim();
  const focal = String(settings.focalT || '').trim().replace(/mm$/i, '');
  const fnumRaw = String(settings.fNumberT || '').trim();
  const shutter = String(settings.shutterT || '').trim();
 ...
 * @returns {string}
 */
export function formatDateT(raw) {
...
export function updateContentPreview(elements, settings = {}) {
  ...
  const L = TYPE_T_LAYOUT;
  const color = resolveTextColor(settings);

  const topLine = String(settings.topLineT || '').trim();
  const handle = String(settings.photoByT || '').trim();
  const date = formatDateT(settings.dateT);
  ...
  let html = `<div class="type-t-layer" style="color:${color}">`;

  if (topLine || handle) {
    html += `<div class="type-t-top" style="left:${topLeftX}px;top:${topLeftY}px;font-size:${topFont}px;letter-spacing:${(L.topLetterSpacing * topFont).toFixed(2)}px;line-height:${L.topLineHeight}">`;
    if (topLine) html += `<div class="type-t-top-line">${escapeHtml(topLine)}</div>`;
    else html += `<div class="type-t-top-line">&nbsp;</div>`;
    if (handle) html += `<div class="type-t-top-line">PHOTO BY ${escapeHtml(handle)}</div>`;
    html += `</div>`;
  }

  if (date) {
    html += `<div class="type-t-right" style="right:${dateRight}px;font-size:${dateFont}px;letter-spacing:${(L.dateLetterSpacing * dateFont).toFixed(2)}px">`;
    html += `<div class="type-t-date">${escapeHtml(date)}</div>`;
    html += `<div class="type-t-mark"></div>`;
    html += `</div>`;
  }

  if (params) {
    html += `<div class="type-t-params" style="right:${paramsRight}px;top:${paramsTop}px;font-size:${paramsFont}px;letter-spacing:${(L.paramsLetterSpacing * paramsFont).toFixed(2)}px">${escapeHtml(params)}</div>`;
  }
...
function updateDragHint() {
  ...
  parent.querySelectorAll('.type-t-drag-hint').forEach(hint => hint.remove());

  const m = getMaxOffset();
  if (m.x > 5 || m.y > 5) {
    const hint = document.createElement('div');
    hint.className = 'type-t-drag-hint';
...
export function reset() {
  ...
  if (state.frameWrapper) {
    state.frameWrapper.classList.remove('type-t');
